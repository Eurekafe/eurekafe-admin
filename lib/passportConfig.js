...
module.exports = function(passport, dbclient) {
  ...
  return function (req, res, next) {
      ...
      dbclient.then((client) => {
        ...
        client.db("eurekafe").collection("users").find(select).toArray((err, response) => {
          ...
          if(response.length != 0) {
            ...
            let insert = {"name": user.displayName};
            insert[idField] = user.id;
            client.db("eurekafe").collection("users").insertOne(insert, (err, resp) => {
              if(err) cb("Identification error =(");
    ...
    if(process.env.TWITTER_CONSUMER_KEY && process.env.TWITTER_CONSUMER_SECRET) {
      let callbackURL = process.env.NODE_ENV == "production" ? "https://eure-admin.herokuapp.com/auth/twitter/callback" : "http://localhost:3001/auth/twitter/callback";
